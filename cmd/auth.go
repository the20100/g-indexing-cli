...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Manage Google Indexing API authentication",
}
...
var authSetupCmd = &cobra.Command{
	...
	Long: `Configure authentication for the Google Indexing API.

─── Option A: Service Account (recommended for automation) ─────────────────
  1. Create a service account in Google Cloud Console
  2. Grant it "Owner" access to the Search Console property
...
func runAuthSetup(cmd *cobra.Command, args []string) error {
	...
	if serviceAccountFile != "" {
		...
		fmt.Printf("  Add %s as an Owner of each Search Console property you want to index.\n", saEmail)
		return nil
	}
...
func runAuthStatus(cmd *cobra.Command, args []string) error {
	...
	case config.AuthMethodOAuth2:
		...
		if cfg.RefreshToken != "" || cfg.AccessToken != "" {
			...
			fmt.Println("Status:      not authenticated — run: g-indexing auth setup")
		}
	default:
		fmt.Println("Auth method: not configured")
		fmt.Println("\nRun: g-indexing auth setup --service-account sa.json")
	}
	return nil
}
